...
avg_profit = sum([line[4] for line in data if line[4] > 0]) / len(data)
print(f"Средняя прибыль без учёта убыточных компаний: {avg_profit}")

firms_dict = {line[0]: line[4] for line in data}
firms_sorted = dict(sorted(firms_dict.items(), key=lambda item: item[1]))
