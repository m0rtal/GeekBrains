...
-- таблица для пометки сообщений как спам   
...
CREATE TABLE spam_messages (
	...
	-- кому сообщение / кто пометил 
    to_user_id BIGINT UNSIGNED,
	
    -- удаляем если сообщения больше нет, но сохраняем если нет больше пользователей - например, для машинного обучения
...
-- таблица для забаненых в сообществах пользователях
...
CREATE TABLE communities_posts (
	...
	INDEX community_user_idx(community_id, from_user_id),
    FOREIGN KEY (community_id) REFERENCES communities(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (from_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE SET NULL,
